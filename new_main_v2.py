...
def main():
    ...
    # Load test data
    df = pd.read_csv(r'/home/atish41/travel_recommder/user_data1.csv')
    
    # Update recommender with data
    recommender.update_from_dataframe(df)
    
    # Test recommendations with timing
    user_id = 67
    ...
    print(f"\nFirst request time: {first_request_time:.4f} seconds")
    ...
    print(f"Cached request time: {cached_request_time:.4f} seconds")
    
    # Print recommendations
    print(f"\nRecommendations for user {user_id}:")
    for rec in recommendations:
        print(f"\nRecommended user: {rec['user_id']}")
        print(f"Mutual connections: {rec['mutual_connection_count']}")
        print(f"Mutual connection IDs: {rec['mutual_connections']}")
